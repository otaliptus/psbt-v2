...
func (u *Updater) addPartialSignature(inIndex int, sig []byte,
	...
	if pInput.NonWitnessUtxo != nil {
		if len(u.Upsbt.Inputs) < inIndex+1 {
			return ErrInvalidPrevOutNonWitnessTransaction
		}
		...
		if pInput.NonWitnessUtxo.TxHash() != prevOut.Hash {
			return ErrInvalidSignatureForInput
		}
		...
		if pInput.RedeemScript != nil {
			...
			if int(outIndex) >= len(pInput.NonWitnessUtxo.TxOut) {
				return ErrInvalidPrevOutNonWitnessTransaction
			}
	...
	// BIP-370: after signing, update PSBT_GLOBAL_TX_MODIFIABLE based on
	// the sighash type used. This only applies to v2 packets.
	if u.Upsbt.Version == 2 {
		u.updateTxModifiable(inIndex)
	}
...
// updateTxModifiable updates the PSBT_GLOBAL_TX_MODIFIABLE flags after a
// signature is added to a v2 packet, per BIP-370:
//
//   - If the sighash does NOT have ANYONECANPAY, inputs are no longer
//     modifiable (clear bit 0).
//   - If the sighash base type is not NONE, outputs are no longer
//     modifiable (clear bit 1).
//   - If the sighash base type is SINGLE, set the Has SIGHASH_SINGLE
//     flag (set bit 2).
func (u *Updater) updateTxModifiable(inIndex int) {
	pInput := u.Upsbt.Inputs[inIndex]

	// If TxModifiable was never set, the Creator chose not to declare
	// modifiability constraints. Don't materialize the field — that would
	// change the serialized packet shape for downstream consumers.
	if u.Upsbt.TxModifiable == nil {
		return
	}

	// Determine the sighash type; default is SIGHASH_ALL.
	sht := txscript.SigHashAll
	if pInput.SighashType != 0 {
		sht = pInput.SighashType
	}

	baseType := sht & 0x1f
	hasACP := sht&txscript.SigHashAnyOneCanPay != 0

	// If not ANYONECANPAY, inputs are locked.
	if !hasACP {
		*u.Upsbt.TxModifiable &^= 0x01
	}

	// If base type is not NONE, outputs are locked.
	if baseType != txscript.SigHashNone {
		*u.Upsbt.TxModifiable &^= 0x02
	}

	// If base type is SINGLE, set the SIGHASH_SINGLE flag.
	if baseType == txscript.SigHashSingle {
		*u.Upsbt.TxModifiable |= 0x04
	}
}
